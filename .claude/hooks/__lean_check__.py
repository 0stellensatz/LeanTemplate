...
`__check__.py` is textual and fast (~40 ms), and it catches what a `lake build`
does not report as an error---above all a source file missing from the root
all-import module, which is then not built, not type-checked, and green.  That is
a *multi-file* invariant, transiently violated by any normal edit sequence---a new
file exists for a moment before its `import` line is added---so checking after
every single edit would fire constantly on states that are merely half-written.
